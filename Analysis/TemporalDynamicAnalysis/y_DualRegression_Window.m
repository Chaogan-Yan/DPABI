function [ResultBrain_AllWindow, TimeSeries, Header] = y_DualRegression_Window(WindowSize, WindowStep, WindowType, AllVolume, SeedMap, OutputName, MaskData, IsNeedDetrend, Band, TR, TemporalMask, ScrubbingMethod, ScrubbingTiming, Header, CUTNUMBER)
...
% Perform dynamic dual regression
% Input:
...
%                       default: 10
% Output:
%	ResultBrain_AllWindow     -   The dual regression map
...
ResultBrain_AllWindow = zeros([nDim1 nDim2 nDim3 nWindow]);
...
for iWindow = 1:nWindow
    ...
    SpatialMean = mean(AllVolumeWindow,2);
    AllVolumeWindowDeSpatialMean = (AllVolumeWindow-repmat(SpatialMean,1,size(AllVolumeWindow,2)));
    TimeSeries=AllVolumeWindowDeSpatialMean*Cov*inv(Cov'*Cov);
    
    %Second regression
    Cov=TimeSeries;
    Cov = (Cov - mean(Cov))/std(Cov); %Mean centering and variance normalization
    TemporalMean = mean(AllVolumeWindow);
    AllVolumeWindowDeTemporalMean = AllVolumeWindow-repmat(TemporalMean,size(AllVolumeWindow,1),1);
    b=inv(Cov'*Cov)*Cov'*AllVolumeWindow;
    ...
    ResultBrain_AllWindow(:,:,:,iWindow) = ResultBrain;
end

Header.pinfo = [1;0;0];
Header.dt    =[16,0];

y_Write(ResultBrain_AllWindow,Header,OutputName);
