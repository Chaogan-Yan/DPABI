function ret = w_spm_ov_roi(varargin)
...
switch cmd
    case 'init'
        % w_spm_ov_roi('init', volhandle, Vroi, loadasroi, xyz)
        spm('pointer','watch');
        ...
        if any(Vroi.dim(1:3)-st{curfig}.vols{volhandle}.dim(1:3))|| ...
                any(Vroi.mat(:)-st{curfig}.vols{volhandle}.mat(:))
        ...
        cb = cell(1,3);
        for k=1:3
            cb{k}=get(st{curfig}.vols{volhandle}.ax{k}.ax,'ButtonDownFcn');
            set(st{curfig}.vols{volhandle}.ax{k}.ax,...
                'ButtonDownFcn',...
                @(ob,ev)w_spm_ov_roi('bdfcn',volhandle,ob,ev));
        end;
        
        st{curfig}.vols{volhandle}.roi = struct('Vroi',Vroi, 'xyz',xyz, 'roi',roi,...
                                        'hroi',1, 'fxyz',fxyz,...
                                        ...
                                        'erothresh',.5);
        st{curfig}.vols{volhandle}.roi.cb = cb;
        
        if ~isempty(st{curfig}.vols{volhandle}.roi.fxyz)
            if isfield(st{curfig}.vols{volhandle}, 'blobs')
                st{curfig}.vols{volhandle}.roi.hframe = numel(st{curfig}.vols{volhandle}.blobs)+1;
            end;
            spm_orthviews('addcolouredblobs',volhandle, ...
                          st{curfig}.vols{volhandle}.roi.fxyz,...
                          ones(size(st{curfig}.vols{volhandle}.roi.fxyz,2),1), ... 
                          st{curfig}.vols{volhandle}.roi.Vroi.mat,[1 .5 .5]);
            st{curfig}.vols{volhandle}.blobs{st{curfig}.vols{volhandle}.roi.hframe}.max=1.3;
        end;
        update_roi=1;
    ...
    case 'edit'
        switch st{curfig}.vols{volhandle}.roi.tool
            case 'box'
                spm('pointer','watch');
                pos = round(inv(st{curfig}.vols{volhandle}.roi.Vroi.mat)* ...
                            [spm_orthviews('pos'); 1]); 
                tmp = round((st{curfig}.vols{volhandle}.roi.box-1)/2);
                [sx sy sz] = meshgrid(-tmp(1):tmp(1), -tmp(2):tmp(2), -tmp(3):tmp(3));
                sel = [sx(:)';sy(:)';sz(:)']+repmat(pos(1:3), 1,prod(2*tmp+1));
                tochange = sel(:, (all(sel>0) &...
                                   sel(1,:)<=st{curfig}.vols{volhandle}.roi.Vroi.dim(1) & ...
                                   sel(2,:)<=st{curfig}.vols{volhandle}.roi.Vroi.dim(2) & ...
                                   sel(3,:)<=st{curfig}.vols{volhandle}.roi.Vroi.dim(3)));
                update_roi = 1;
                
            case 'connect'
                w_spm_ov_roi('connect', volhandle)
      ...
      %             implied warranty.
                for k = 1:3
                    if st{curfig}.vols{volhandle}.ax{k}.ax == gca
                        axhandle = k;
                        break;
                    end;
                end;
                line_color = [1 1 0];
                axes(st{curfig}.vols{volhandle}.ax{axhandle}.ax);
      
                hold on;
                Xlimits = get (st{curfig}.vols{volhandle}.ax{axhandle}.ax,'XLim');
                Ylimits = get (st{curfig}.vols{volhandle}.ax{axhandle}.ax,'YLim');
                XLimMode = get(st{curfig}.vols{volhandle}.ax{axhandle}.ax,'XLimMode');
                set(st{curfig}.vols{volhandle}.ax{axhandle}.ax,'XLimMode','manual');
                YLimMode = get(st{curfig}.vols{volhandle}.ax{axhandle}.ax,'YLimMode');
                set(st{curfig}.vols{volhandle}.ax{axhandle}.ax,'YLimMode','manual');
                ButtonDownFcn = get(st{curfig}.vols{volhandle}.ax{axhandle}.ax,'ButtonDownFcn');
                set(st{curfig}.vols{volhandle}.ax{axhandle}.ax,'ButtonDownFcn','');
                UIContextMenu = get(st{curfig}.vols{volhandle}.ax{axhandle}.ax,'UIContextMenu');
                set(st{curfig}.vols{volhandle}.ax{axhandle}.ax,'UIContextMenu',[]);
                set(st{curfig}.vols{volhandle}.ax{axhandle}.ax,'Selected','on');
                ...
                while ~isempty(bc)
                    ...
                    if isempty(xc) || bc > 1
                        ...
                        lineHandle = line (x,y,ones(1,length(x)), ...
                                           'Color',line_color,...
                                           'parent',st{curfig}.vols{volhandle}.ax{axhandle}.ax,...
                                           'HitTest','off');
                    end;
                end
                ...
                if ~isempty(x)
                    ...
                    x(i)=x(1);
                    y(i)=y(1);
                    prms=spm_imatrix(st{curfig}.vols{volhandle}.roi.Vroi.mat);
                    ...
                    switch axhandle
                        case 1,
                            M0 = [ 1 0 0 -st{curfig}.bb(1,1)+1
                                   0 1 0 -st{curfig}.bb(1,2)+1
                                   0 0 1 -cent(3)
                                   0 0 0 1];
                            polyoff(3) = st{curfig}.vols{volhandle}.roi.polyslices/2;
                            polythick = prms(9);
                        case 2,
                            M0 = [ 1 0 0 -st{curfig}.bb(1,1)+1
                                   0 0 1 -st{curfig}.bb(1,3)+1
                                   0 1 0 -cent(2)
                                   0 0 0 1];
                            polyoff(2) = st{curfig}.vols{volhandle}.roi.polyslices/2;
                            polythick = prms(8);
                        case 3,
                            if st{curfig}.mode ==0,
                                M0 = [ 0 0 1 -st{curfig}.bb(1,3)+1
                                       0 1 0 -st{curfig}.bb(1,2)+1
                                       1 0 0 -cent(1)
                                       0 0 0 1];
                            else
                                M0 = [ 0 -1 0 +st{curfig}.bb(2,2)+1
                                       0  0 1 -st{curfig}.bb(1,3)+1
                                       1  0 0 -cent(1)
                                       0  0 0 1];
                            end;
                            polyoff(1) = st{curfig}.vols{volhandle}.roi.polyslices/2;
                            polythick = abs(prms(7));
                    end;
                    polvx = inv(st{curfig}.vols{volhandle}.roi.Vroi.mat)*st{curfig}.Space*inv(M0)*...
                    ...
                    [xbox ybox zbox] = ndgrid(max(min(floor(polvx(1,:)-polyoff(1))),1):...
                                              min(max(ceil(polvx(1,:)+polyoff(1))),...
                                                  st{curfig}.vols{volhandle}.roi.Vroi.dim(1)),...
                                              max(min(floor(polvx(2,:)-polyoff(2))),1):...
                                              min(max(ceil(polvx(2,:)+polyoff(2))),...
                                                  st{curfig}.vols{volhandle}.roi.Vroi.dim(2)),...
                                              max(min(floor(polvx(3,:)-polyoff(3))),1):...
                                              min(max(ceil(polvx(3,:)+polyoff(3))),...
                                                  st{curfig}.vols{volhandle}.roi.Vroi.dim(3)));
                    % re-transform in polygon plane
                    xyzbox = M0*is*st{curfig}.vols{volhandle}.roi.Vroi.mat*[xbox(:)';ybox(:)';zbox(:)';...
                                        ones(size(xbox(:)'))];
                    xyzbox = xyzbox(:,abs(xyzbox(3,:))<=.6*polythick*...
                                    st{curfig}.vols{volhandle}.roi.polyslices); % nearest neighbour to polygon
                    sel = logical(inpolygon(xyzbox(1,:),xyzbox(2,:),x,y));
                    xyz = inv(st{curfig}.vols{volhandle}.roi.Vroi.mat)*st{curfig}.Space*inv(M0)*xyzbox(:,sel);
                    if ~isempty(xyz)
                        tochange = round(xyz(1:3,:));
                        update_roi = 1;
                    end;
                end;
                set(st{curfig}.vols{volhandle}.ax{axhandle}.ax,...
                    'Selected','off', 'XLimMode',XLimMode, 'YLimMode',YLimMode,...
                    'ButtonDownFcn',ButtonDownFcn, 'UIContextMenu',UIContextMenu);
        end;
    case 'thresh'
        spm('pointer','watch');
        rind = find(st{curfig}.vols{volhandle}.roi.roi);
        [x y z]=ind2sub(st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3),rind);
        tmp = round(inv(st{curfig}.vols{volhandle}.mat) * ...
                    st{curfig}.vols{volhandle}.roi.Vroi.mat*[x'; y'; z'; ones(size(x'))]); 
        dat = spm_sample_vol(st{curfig}.vols{volhandle}, ...
                             tmp(1,:), tmp(2,:), tmp(3,:), 0);
        sel = ~((st{curfig}.vols{volhandle}.roi.thresh(1) < dat) & ...
                (dat < st{curfig}.vols{volhandle}.roi.thresh(2)));
        if strcmp(st{curfig}.vols{volhandle}.roi.mode,'set')
            toclear = [x(sel)'; y(sel)'; z(sel)'];
        else
            toset   = [x(sel)'; y(sel)'; z(sel)'];
            toclear = st{curfig}.vols{volhandle}.roi.xyz;
        end;
        update_roi = 1;
        
    case 'erodilate'
        spm('pointer','watch');
        V = zeros(size(st{curfig}.vols{volhandle}.roi.roi));
        spm_smooth(double(st{curfig}.vols{volhandle}.roi.roi), V, 2);
        [ero(1,:) ero(2,:) ero(3,:)] = ind2sub(st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3),...
                                               find(V(:)>st{curfig}.vols{volhandle}.roi.erothresh));
        if strcmp(st{curfig}.vols{volhandle}.roi.mode,'set')
            toset   = ero;
            toclear = st{curfig}.vols{volhandle}.roi.xyz;
        else
    ...
    case {'connect', 'cleanup'}
        spm('pointer','watch');    
        [V L] = spm_bwlabel(double(st{curfig}.vols{volhandle}.roi.roi),6);
        sel = [];
        switch cmd
            case 'connect'
                pos = round(inv(st{curfig}.vols{volhandle}.roi.Vroi.mat)* ...
            ...
            case 'cleanup'
                ...
                sel = find(numV>st{curfig}.vols{volhandle}.roi.csize);
        end;
        if ~isempty(sel)
            ...
            [conn(1,:) conn(2,:) conn(3,:)] = ...
                ind2sub(st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3),ind);
    ...
    case 'invert'
        spm('pointer','watch');
        st{curfig}.vols{volhandle}.roi.roi = ~st{curfig}.vols{volhandle}.roi.roi;
        ind = find(st{curfig}.vols{volhandle}.roi.roi);
        st{curfig}.vols{volhandle}.roi.xyz = zeros(3,numel(ind));
        [st{curfig}.vols{volhandle}.roi.xyz(1,:), ...
            st{curfig}.vols{volhandle}.roi.xyz(2,:), ...
            st{curfig}.vols{volhandle}.roi.xyz(3,:)] = ind2sub(st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3),ind);
        update_roi = 1;
        
    case 'clear'
        spm('pointer','watch');
        st{curfig}.vols{volhandle}.roi.roi = false(size(st{curfig}.vols{volhandle}.roi.roi));
        st{curfig}.vols{volhandle}.roi.xyz=[];
        update_roi = 1;
        
    case 'addfile'
        V = spm_vol(spm_select([1 Inf],'image','Image(s) to add'));
        [x y z] = ndgrid(1:st{curfig}.vols{volhandle}.roi.Vroi.dim(1),...
                         1:st{curfig}.vols{volhandle}.roi.Vroi.dim(2),...
                         1:st{curfig}.vols{volhandle}.roi.Vroi.dim(3));
        xyzmm = st{curfig}.vols{volhandle}.roi.Vroi.mat*[x(:)';y(:)';z(:)'; ...
                            ones(1, prod(st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3)))];
        msk = false(1,prod(st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3)));
        for k = 1:numel(V)
        ...
        [tochange(1,:) tochange(2,:) tochange(3,:)] = ind2sub(st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3),find(msk));
        clear xyzmm xyzvx msk
        update_roi = 1;
        
    case {'save','saveas'}
        if strcmp(cmd,'saveas') || ...
                exist(st{curfig}.vols{volhandle}.roi.Vroi.fname, 'dir')
            flt = {'*.nii','NIfTI (1 file)';'*.img','NIfTI (2 files)'};
            [name pth idx] = uiputfile(flt, 'Output image');
            if ~ischar(pth)
                warning('spm:w_spm_ov_roi','Save cancelled');
                return;
            end;
            ...
            st{curfig}.vols{volhandle}.roi.Vroi.fname = fullfile(p, [n e v]);
        end;
        spm('pointer','watch');
        spm_write_vol(st{curfig}.vols{volhandle}.roi.Vroi, ...
                      st{curfig}.vols{volhandle}.roi.roi);
        spm('pointer','arrow');
        return;
 ...
 % Context menu and callbacks
    case 'context_menu'  
        ...
        iorient = findobj(st{curfig}.vols{volhandle}.ax{1}.cm, 'Label', 'Orientation');
    ...
    case 'context_init'
        ...
        spm_input('!DeleteInputObj',Finter);
        feval('w_spm_ov_roi','init',volhandle,imfname,loadasroi,xyz);
        return;
        
    case 'context_selection'
        st{curfig}.vols{volhandle}.roi.mode = varargin{3};
    ...
    case 'context_edit'
        st{curfig}.vols{volhandle}.roi.tool = varargin{3};
    ...
    case 'context_box'
        ...
        box = spm_input('Selection size {vx vy vz}','!+1','e', ...
                        num2str(st{curfig}.vols{volhandle}.roi.box), [1 3]);
        spm_input('!DeleteInputObj',Finter);
        st{curfig}.vols{volhandle}.roi.box = box;
        return;
    ...
    case 'context_polyslices'
        ...
        polyslices = spm_input('Polygon: slices around current slice','!+1','e', ...
                               num2str(st{curfig}.vols{volhandle}.roi.polyslices), [1 1]);
        spm_input('!DeleteInputObj',Finter);
        st{curfig}.vols{volhandle}.roi.polyslices = polyslices;
        return;
    ...
    case 'context_csize'
        ...
        csize = spm_input('Minimum cluster size (#vx)','!+1','e', ...
                          num2str(st{curfig}.vols{volhandle}.roi.csize), [1 1]);
        spm_input('!DeleteInputObj',Finter);
        st{curfig}.vols{volhandle}.roi.csize = csize;
        return;
    ...
    case 'context_erothresh'
        ...
        erothresh = spm_input('Erosion/Dilation threshold','!+1','e', ...
                              num2str(st{curfig}.vols{volhandle}.roi.erothresh), [1 1]);
        spm_input('!DeleteInputObj',Finter);
        st{curfig}.vols{volhandle}.roi.erothresh = erothresh;
        return;
        
    case 'context_space'
        spm_orthviews('space', volhandle, ...
              st{curfig}.vols{volhandle}.roi.Vroi.mat, ...
              st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3));
    ...
    case 'context_thresh'
        ...
        thresh = spm_input('Threshold  {min max}','!+1','e', ...
                           num2str(st{curfig}.vols{volhandle}.roi.thresh), [1 2]);
        spm_input('!DeleteInputObj',Finter);
        st{curfig}.vols{volhandle}.roi.thresh = thresh;
        feval('w_spm_ov_roi', 'thresh', volhandle);
        return;
    ...
    case 'context_quit'
        ...
        for k=1:3
            set(st{curfig}.vols{volhandle}.ax{k}.ax,'ButtonDownFcn', st{curfig}.vols{volhandle}.roi.cb{k});
        end;
        st{curfig}.vols{volhandle} = rmfield(st{curfig}.vols{volhandle}, 'roi');
    ...
    case 'bdfcn'
        if strcmpi(get(gcf,'SelectionType'), 'extend')
            ...
            for k=1:3
                if isequal(st{curfig}.vols{volhandle}.ax{k}.ax, gca)
                    break;
                end
            end
            feval(st{curfig}.vols{volhandle}.roi.cb{k});
        end
    otherwise    
...
if update_roi
    ...
    if ~isempty(toclear)
        itoclear = sub2ind(st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3), ...
                           toclear(1,:), toclear(2,:), toclear(3,:)); 
        st{curfig}.vols{volhandle}.roi.roi(itoclear) = false;
        if ~isempty(st{curfig}.vols{volhandle}.roi.xyz)
            st{curfig}.vols{volhandle}.roi.xyz = setdiff(st{curfig}.vols{volhandle}.roi.xyz',toclear','rows')';
        else
            st{curfig}.vols{volhandle}.roi.xyz = [];
        end;
    end;
    ...
    if ~isempty(toset)
        ...
        % it is necessary
        itoset = round(sub2ind(st{curfig}.vols{volhandle}.roi.Vroi.dim(1:3), ...
                               toset(1,:), toset(2,:), toset(3,:))); 
        st{curfig}.vols{volhandle}.roi.roi(itoset) = true;
        if ~isempty(st{curfig}.vols{volhandle}.roi.xyz)
            st{curfig}.vols{volhandle}.roi.xyz = union(st{curfig}.vols{volhandle}.roi.xyz',toset','rows')';
        else
            st{curfig}.vols{volhandle}.roi.xyz = toset;
        end;
    end;
    
    if isfield(st{curfig}.vols{volhandle}, 'blobs')
        nblobs=length(st{curfig}.vols{volhandle}.blobs);
        if nblobs>1
            blobstmp(1:st{curfig}.vols{volhandle}.roi.hroi-1) = st{curfig}.vols{volhandle}.blobs(1:st{curfig}.vols{volhandle}.roi.hroi-1);
            blobstmp(st{curfig}.vols{volhandle}.roi.hroi:nblobs-1) = st{curfig}.vols{volhandle}.blobs(st{curfig}.vols{volhandle}.roi.hroi+1:nblobs);
            st{curfig}.vols{volhandle}.blobs=blobstmp;
        else
            if isempty(st{curfig}.vols{volhandle}.roi.hframe) % save frame
                st{curfig}.vols{volhandle}=rmfield(st{curfig}.vols{volhandle},'blobs');
            end;
        end;
    end;
    if isfield(st{curfig}.vols{volhandle}, 'blobs')
        st{curfig}.vols{volhandle}.roi.hroi = numel(st{curfig}.vols{volhandle}.blobs)+1;
    else
        st{curfig}.vols{volhandle}.roi.hroi = 1;
    end;
    if isempty(st{curfig}.vols{volhandle}.roi.xyz)  % initialised with empty roi
        spm_orthviews('addcolouredblobs', volhandle, ...
                      [1; 1; 1], 0, st{curfig}.vols{volhandle}.roi.Vroi.mat,[1 3 1]); 
    else
        spm_orthviews('addcolouredblobs', volhandle, ...
                      st{curfig}.vols{volhandle}.roi.xyz, ones(size(st{curfig}.vols{volhandle}.roi.xyz,2),1), ... 
                      st{curfig}.vols{volhandle}.roi.Vroi.mat,[1 3 1]); % use color that is more intense than standard rgb range
    end;
    st{curfig}.vols{volhandle}.blobs{st{curfig}.vols{volhandle}.roi.hroi}.max=2;
    spm_orthviews('redraw');
end;
