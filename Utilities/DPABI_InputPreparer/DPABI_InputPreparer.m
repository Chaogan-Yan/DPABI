...
if handles.Cfg.InputLayout == 1 % Participant first
    ...
    if ~handles.Cfg.AnatOnly
        ...
        if handles.Cfg.SeriesFileNumber.LowLimitMode.Fun % Do not fix the number of dicom files, setting a lower threshold for the number. Bin Lu, 20220921.
            nFileFun = handles.Cfg.SeriesFileNumber.LowThreshold.Fun;
            try
                FunStatus = cellfun(@(Series,Sub) ...
                    (length(java.io.File([handles.Cfg.WorkingDir,filesep,Sub,filesep,Series]).listFiles)-nFileFun)>=0,...
                    FunString,SubString_Fun,'UniformOutput', false);
            catch
                FunStatus = cellfun(@(Series,Sub) ...
                    (length(dir([handles.Cfg.WorkingDir,filesep,Sub,filesep,Series]))-handles.Cfg.nFileOperator-nFileFun)>=0,...
                    FunString,SubString_Fun,'UniformOutput', false);
            end
        else
        ...
        if handles.Cfg.IsOtherFun
            ...
            for iSession = 1:handles.Cfg.FunOtherNumber
                ...
                if handles.Cfg.SeriesFileNumber.LowLimitMode.FunOther{iSession} % Do not fix the number of dicom files, setting a lower threshold for the number. Bin Lu, 20220921.
                    nFileFunOther = handles.Cfg.SeriesFileNumber.LowThreshold.Fun{iSession};
                    try
                        FunOtherStatus{iSession} = cellfun(@(Series,Sub) ...
                            (length(java.io.File([handles.Cfg.WorkingDir,filesep,Sub,filesep,Series]).listFiles)-nFileFunOther)>=0,...
                            FunOtherString{iSession},SubString_FunOther{iSession},'UniformOutput', false);
                    catch
                        FunOtherStatus{iSession} = cellfun(@(Series,Sub) ...
                            (length(dir([handles.Cfg.WorkingDir,filesep,Sub,filesep,Series]))-handles.Cfg.nFileOperator-nFileFunOther)>=0,...
                            FunOtherString{iSession},SubString_FunOther{iSession},'UniformOutput', false);
                    end
                else
    ...
    if ~handles.Cfg.AnatOnly
        ...
        if handles.Cfg.SeriesFileNumber.LowLimitMode.Fun % Do not fix the number of dicom files, setting a lower threshold for the number. Bin Lu, 20220921.
            nFileFun = handles.Cfg.SeriesFileNumber.LowThreshold.Fun;
            FunStatus = cellfun(@(Series,Sub) ...
                (length(dir([handles.Cfg.WorkingDir,filesep,Series,filesep,Sub]))-handles.Cfg.nFileOperator-nFileFun)>=0,...
                FunString,SubString_Fun,'UniformOutput', false);
        else
            nFileFun = handles.Cfg.SeriesFileNumber.List.Fun(handles.Cfg.SeriesFileNumber.Flag.Fun);
            FunStatus = cellfun(@(Series,Sub) ...
        ...
        if handles.Cfg.IsOtherFun
            ...
            if handles.Cfg.SeriesFileNumber.LowLimitMode.FunOther{iSession} % Do not fix the number of dicom files, setting a lower threshold for the number. Bin Lu, 20220921.
                for iSession = 1:handles.Cfg.FunOtherNumber
                    nFileFunOther = handles.Cfg.SeriesFileNumber.LowThreshold.FunOther{iSession};
                    FunOtherStatus{iSession} = cellfun(@(Series,Sub) ...
                        (length(dir([handles.Cfg.WorkingDir,filesep,Series,filesep,Sub]))-handles.Cfg.nFileOperator-nFileFunOther)>=0,...
                        FunOtherString{iSession},SubString_FunOther{iSession},'UniformOutput', false);
                end
            else
                for iSession = 1:handles.Cfg.FunOtherNumber
