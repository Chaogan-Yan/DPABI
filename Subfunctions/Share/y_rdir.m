function [varargout] = y_rdir(rootdir,varargin)
...
elseif strcmp(wildpath,'**')
  ...
  D = y_rdir([prepath postpath(2:end)]);
  ...
  c_D = arrayfun(@(x) y_rdir([prepath x.name filesep wildpath postpath]),...
  ...
  c_D = arrayfun(@(x) y_rdir([prepath x.name postpath]),...
    D_sd, 'UniformOutput', false);
